...
function getMetrikaCounter() {
    try {
        return eval('yaCounter' + Ya.Metrika.counters()[0].id);
    } catch (e) {
        return false;
    }
}
...
function trackGoal(goal, params) {
    if (typeof(yaCounter) != 'undefined' || (yaCounter = getMetrikaCounter())) {
        yaCounter.reachGoal(goal, params);
    }
    if (goal == 'ORDER') {
        if (typeof(ga) == 'function') {
            ga('send', 'event', ((typeof(ga_event1) == 'undefined') ? 'order' : ga_event1), ((typeof(ga_event2) == 'undefined') ? 'submit' : ga_event2));
        }
    }
    if (goal == 'CBK_CALL') {
        if (typeof(ga) == 'function') {
            ga('send', 'event', 'CallbackKILLER', 'NewCall');
        }
    }
}
...
function getUrlVars() {
    var vars = [],
        hash;
    var hashes = window.location.href.slice(window.location.href.indexOf('?') + 1, window.location.href.indexOf('#') >= 0 ? window.location.href.indexOf('#') : undefined).split('&');
...
function saveSource() {
    ...
    if (url_params['utm_source']) {
        $.cookie('utm_source', url_params['utm_source'], {
            expires: 30,
            path: '/'
        });
    ...
    if (url_params['utm_campaign']) {
        $.cookie('utm_campaign', url_params['utm_campaign'], {
            expires: 30,
            path: '/'
        });
    };
    if (url_params['utm_medium']) {
        $.cookie('utm_medium', url_params['utm_medium'], {
            expires: 30,
            path: '/'
        });
    };
    if (url_params['utm_term']) {
        $.cookie('utm_term', url_params['utm_term'], {
            expires: 30,
            path: '/'
        });
    };
    if (url_params['utm_content']) {
        $.cookie('utm_content', url_params['utm_content'], {
            expires: 30,
            path: '/'
        });
    };
    try {
        if (typeof($.cookie('first_referrer')) == 'undefined') {
            $.cookie('first_referrer', document.referrer, {
                expires: 30,
                path: '/'
            });
        }
...
function showVideo(link) {
    var l = $(link);
    var title = l.data('title');
    var youtube_id = l.data('youtube-id');
    var youtube_list = l.data('youtube-list');
    var youtube_url = 'https://www.youtube.com/embed/' + youtube_id + '?rel=0&controls=1&showinfo=0&autoplay=1';
    if (youtube_list) {
        youtube_url = youtube_url + '&loop=1&listType=playlist&list=' + youtube_list;
    }
    var video = '<div class="col-sm-12"><div class="section-header text-center">';
    video += '<h2>' + title + '</h2>';
    video += '</div></div>';
    video += '<div class="col-sm-12"><div class="video modal-clear"><video autoplay="autoplay" controls="controls" width="100%"><source src=' + youtube_id + '></video></div></div>';
    showModal(video, 770);
    $("#modal_content .video").fitVids();
}
...
function profileObjectCheck() {
    ...
    var object_type = $('#f6441').val();
    $('.f-o').hide();
    if (object_type == 'недвижимость') {
        $('#f6461').parents('.field-group').hide();
        $('#f6451').parents('.field-group').show();
        $('#f6431').parents('.field-group').show();
        var re_type = o_types[$('#f6451').val()];
        $('.f-o.f-real-estate').show();
        if ($.inArray(re_type, ['flat', 'room', 'house']) >= 0) {
            $('.f-o.f-living').show();
        }
        $('.f-o.f-' + re_type).show();
    } else if (object_type == 'транспорт') {
        $('#f6451').parents('.field-group').hide();
        $('#f6461').parents('.field-group').show();
        $('#f6431').parents('.field-group').hide();
        var tp_type = o_types[$('#f6461').val()];
        $('.f-o.f-transport').show();
        $('.f-o.f-' + tp_type).show();
    } else {
        $('#f6461').parents('.field-group').hide();
        $('#f6451').parents('.field-group').hide();
...
function saveProfile(step) {
    ...
    data['lead_hash'] = $.cookie('lead_hash');
    if (!data['lead_hash']) {
        alert('Сначала отправьте заявку!');
        return false;
    }
    if ($('.app-form').length > 0) {
        var url = 'https://app.goldinvest.com.ua/front/site/profile?step=' + step;
    } else {
        var url = 'https://app.goldinvest.com.ua/add.php?from=site-profile&step=' + step;
    }
...
function notifyBase() {
    ...
    if ($('.app-form').length > 0) {
        var url = 'https://app.goldinvest.com.ua/front/site/docs';
    } else {
        var url = 'https://app.goldinvest.com.ua/add.php?from=site-docs';
    }
    $.ajax({
        type: 'POST',
        url: url,
...
function uploadProfileDocs() {
    var form = $('#profile');
    var files = $('#profile-docs', form)[0].files;
    if (files.length == 0) {
        return false;
    } else {
        var ext_whitelist = ['jpeg', 'jpg', 'pdf'];
        var type_whitelist = ['image/jpeg', 'image/jpg', 'application/pdf'];
        var errors = [];
        var extension = '';
        $.each(files, function(key, file) {
            extension = file['name'].split('.')[file['name'].split('.').length - 1].toLowerCase();
            if (type_whitelist.indexOf(file['type']) == -1) {
                errors.push(file['name'] + ' - недопустимый тип файла');
            } else {
                if (ext_whitelist.indexOf(extension) == -1) errors.push(file['name'] + ' - недопустимое расширение файла');
            }
            if (file['size'] > 20971520) errors.push(file['name'] + ' - размер файла не должен превышать 20МБ');
        });
        if (errors.length > 0) {
            alert("При загрузке файлов возникли ошибки:\r\n" + errors.join("; \r\n"));
            return false;
        }
    }
    ...
    $.ajax({
        ...
        contentType: false,
        beforeSend: function() {
            var button = $('.step5 button.form-button.next', form);
            $('.fa.fa-upload', button).addClass('fa-spinner fa-spin').removeClass('fa-upload');
            $('.form-button-text', button).html('Загрузка...');
            $('.step5 button.form-button', form).prop('disabled', true);
        },
        success: function(response, textStatus, jqXHR) {
            ...
            $.each(response, function(index, file) {
                if (file['errors'].length == 0) {
                    html += '<li>' + $("<div>").text(file['name']).html() + '</li>';
                } else {
                    if (errors == '') {
                        errors += "При загрузке файлов возникли ошибки:\r\n";
                    }
                    errors += file['name'] + ': ' + file['errors'].join(', ') + ";\r\n";
                }
            });
            if (html != '') {
                $('.upload-list', form).append(html);
                trackGoal('PROFILE-DOCS');
                if ($.cookie('lead_step') != '3') {
                    $('.uploaded-files', form).show();
                    $('.step5 button.form-button.finish', form).show();
                    $.cookie('lead_step', '3', {
                        expires: 1,
                        path: '/'
                    });
            ...
            $('.form-button-text', button).html('Загружено!');
            setTimeout(function() {
                $('.fa.fa-check', button).addClass('fa-upload').removeClass('fa-check');
                $('.form-button-text', button).html('Добавить');
            }, 2000);
            $('#profile-docs').val('', form);
            $('.step5 button.form-button', form).prop('disabled', false);
            if (errors != '') {
                alert(errors);
            }
        },
        error: function(jqXHR, textStatus, errorThrown) {
            ...
            var button = $('.step5 button.form-button.next', form);
            $('.fa.fa-spinner', button).addClass('fa-upload').removeClass('fa-spinner fa-spin');
            $('.form-button-text', button).html('Загрузить');
            $('.step5 button.form-button', form).prop('disabled', false);
            $('#profile-docs').val('', form);
            alert('При загрузке возникла ошибка. Попробуйте еще раз.');
        }
    });
}
...
function profileNextStep() {
    ...
    if (current) {
        var rfields = $('.profile-step.step' + current + ' .field-group.required-field:visible .field-value').filter(function() {
            return ($(this).prop('type') == 'checkbox' ? !$(this).prop('checked') : $(this).val() == "");
        });
        if (rfields.length == 0 || next < current || next == 7) {
            if (next >= 2 && next <= 5 && next > current) {
                saveProfile(current);
            } else if (next == 6) {
                uploadProfileDocs();
            } else {
                $('.profile-step.step' + current).hide();
                $('.profile-step.step' + next).show();
                $('html, body').scrollTo($('#profile').offset()['top'] - 100, 500);
                if (next == 7) {
                    trackGoal('PROFILE-FINISH');
                    $.removeCookie('lead_hash', {
                        path: '/'
                    });
                    ...
                    trackGoal('PROFILE-STEP' + next);
                }
            }
        } else {
            rfields.parents('.field-group').addClass('p-error');
            $('html, body').scrollTo($(rfields[0]).offset()['top'] - 150, 500);
        }
    } else {
        $('.profile-step.step' + next).show();
        $('html, body').scrollTo($('#profile').offset()['top'] - 100, 500);
...
function initProfileForm(step) {
    if (!step) {
        step = 1;
    }
    var profile_url = ($('#profile').data('url') || '/profile/get_form3/');
    $.ajax({
        type: 'GET',
        url: profile_url,
        success: function(response) {
            $('#profile').html(response);
            $('#profile .step' + step).show();
            $('.profile .prev,.profile .next').click(profileNextStep);
            $('#f6441, #f6451, #f6461').change(profileObjectCheck);
            profileObjectCheck();
            $('#f7151').click(loanIsOwner);
            loanIsOwner();
            $('.profile .field-value').on('input change', profileFieldChanged);
            if ($.cookie('lead_hash')) {
                if ($('.app-form').length > 0) {
                    var url = 'https://app.goldinvest.com.ua/front/site/profile/' + $.cookie('lead_hash') + '?from=site';
                } else {
                    var url = 'https://app.goldinvest.com.ua/get_profile.php?from=site&hash=' + $.cookie('lead_hash');
                }
                $.ajax({
                    url: url,
                    type: 'GET',
...
function initEva() {
    ...
    $(document).scroll(function() {
        if (!evaLoaded) {
            evaLoaded = true;
            $.getScript('/wp-content/themes/creditors/js/eva-v2.js');
        }
    });
}
    ...
    var fn = {
        ...
        YandexMaps: function() {
            ...
            $(document).scroll(function() {
                if (!ymapsLoaded) {
                    ymapsLoaded = true;
                    $.getScript('https://api-maps.yandex.ru/2.1/?apikey=04504854-94cf-45b2-a2b2-e501eeecbcb8&lang=ru_RU', function() {
                        ymaps.ready(function() {
                            function initMap(mCenter, mZoom) {
                                ...
                                if ($('.map-address').length > 0) {
                                    points = $('.map-address').map(function(i, el) {
                                        var pCenter = [$(el).data('lat'), $(el).data('long')];
                                        return new ymaps.Placemark(pCenter, {
                                            balloonContent: $(el).text().trim(),
                                            hintContent: $(el).text().trim()
                                        }, {
                                    ...
                                    $('.map-address').click(function(event) {
                                        event.preventDefault();
                                        var pCenter = [$(this).data('lat'), $(this).data('long')];
                                        myMap.setCenter(pCenter);
                                        myMap.setZoom(17);
                                    });
                                } else {
                                    var myPoint = new ymaps.Placemark(mCenter, {
                                        balloonContent: $('#office_address').text().trim(),
                                        hintContent: $('#office_address').text().trim()
                                    }, {
                                        preset: 'islands#redMoneyIcon'
                                    });
                                    points.push(myPoint);
                            ...
                            var mContainer = $('#map_canvas');
                            var mLat = mContainer.data('lat');
                            var mLong = mContainer.data('long');
                            var mZoom = mContainer.data('zoom');
                            if (!mZoom) {
                                mZoom = 16;
                            }
                            if (mLat && mLong) {
                                var mCenter = [mLat, mLong];
                                initMap(mCenter, mZoom);
                            } else {
                                ymaps.geocode($('#office_address').text().trim().replace(/\s+/g, ' '), {
                                    results: 1
                                }).then(function(res) {
                                    var point = res.geoObjects.get(0),
                                        mCenter = point.geometry.getCoordinates();
                                    initMap(mCenter, mZoom);
                                });
                            }
                        });
        ...
        ClientbaseForm: function() {
            $('.clientbase-form').submit(function(e) {
                var form = $(this);
                e.preventDefault();
                var name = $('.form-name', form).val(),
                    phone = $('.form-telephone', form).val(),
                    email = $(".form-email", form).val(),
                    sum = $(".form-sum", form).val(),
                    ticket = $('.form-ticket', form).val();
                name = name ? name : '';
                phone = phone ? phone : '';
                email = email ? email : "";
                ...
                if (!email || isValidEmail(email)) {
                    $('input.form-email', form).removeClass('not-valid');
                } else {
                    formValid = false;
                    $('input.form-email', form).addClass('not-valid');
                }
                if ($('.form-birth', form).length == 1) {
                    var birthField = $('.form-birth', form);
                    var birth = birthField.val();
                    if (!birth && !birthField.attr('required') || /(0[1-9]|[12][0-9]|3[01])\.(0[1-9]|1[012])\.(19|20)\d\d/.test(birth)) {
                        birthField.removeClass('not-valid');
                    } else {
                        formValid = false;
                        birthField.addClass('not-valid');
                        alert('Формат даты рождения должен быть ДД.ММ.ГГГГ');
                    }
                } else {
                    var birth = '';
                }
        ...
        AppForm: function() {
            $('.app-form').submit(function(e) {
                var form = $(this); 
                var name = $('.form-name', form).val(),
                    city = $('.form-city', form).val(),
                    phone = $('.form-telephone', form).val(),
                    email = $(".form-email", form).val(),
                    sum = $(".form-sum", form).val(),
                    ticket = $('.form-ticket', form).val();
                name = name ? name : '';
                city = city ? city : (mainCity ? mainCity : '');
                phone = phone ? phone : '';
                email = email ? email : "";
                ticket = ticket ? ticket : '';
                var formValid = true;
        ...
        RegisterForm: function() {
            $(".register-form").submit(function(e) {
                ...
                e.preventDefault();
                var name = $(".form-name", form).val(),
                    telephone = $(".form-telephone", form).val(),
                    email = $(".form-email", form).val(),
                    sum = $(".form-sum", form).val(),
                    ticket = $(".form-ticket", form).val();
                name = name ? name : "";
...
function calcPriceRangeInput(fromPayment) {
    ...
    var max_fee = val - $('#calc_sum_range').attr('min');
    $('#calc_fee_range').attr('max', max_fee);
    $('#max_fee').html('до ' + addSpaces(max_fee) + ' ₴');
    if ($('#calc_min_fee').length > 0) {
        var min_fee = val * $('#calc_min_fee').val();
        $('#calc_fee_range').attr('step', 1);
        $('#calc_fee_range').attr('min', min_fee);
        $('#min_fee').html('от ' + addSpaces(min_fee) + ' ₴');
        var max_sum = val - min_fee;
        $('#calc_sum_range').attr('max', max_sum);
        $('#max_sum').html('до ' + addSpaces(max_sum) + ' ₴');
    } else {
        $('#calc_sum_range').attr('max', val);
        $('#max_sum').html('до ' + addSpaces(val) + ' ₴');
    }
...
function calcTermRangeInput() {
    ...
    $('#calc_term').val(addSpaces(val) + ' мес.');
    calcResult();
}
...
function calcPaymentRangeInput() {
    ...
    ir.attr('max', Math.ceil(ir.attr('max') / 1000) * 1000);
    var payment = ir.val();
    var term = parseFloat($('#calc_term_range').val());
    var rate = parseFloat($('#calc_rate_range').val() / (12 * 100));
    var payment_type = $('#calc_payment_type').val();
    if (payment_type == '1') {
...
function calcResult(fromPayment) {
    var sum = parseFloat($('#calc_sum_range').val());
    var min_sum = $('#calc_sum_range').attr('min');
    var max_sum = $('#calc_sum_range').attr('max');
    var term = parseFloat($('#calc_term_range').val());
    var min_term = $('#calc_term_range').attr('min');
    var max_term = $('#calc_term_range').attr('max');
    var rate = parseFloat($('#calc_rate_range').val() / (12 * 100));
    var min_rate = $('#calc_rate_range').attr('min') / (12 * 100);
    var max_rate = $('#calc_rate_range').attr('max') / (12 * 100);
    var payment_type = $('#calc_payment_type').val();
    if (payment_type == '1') {
        var kef = (rate + (rate / (Math.pow(1 + rate, term) - 1)));
    ...
    $('#min_payment').html('от ' + addSpaces(Math.round(min_payment)) + ' ₴');
    $('#max_payment').html('до ' + addSpaces(Math.round(max_payment)) + ' ₴');
...
function showShedule() {
    ...
    table += 'Срок: <strong>' + term + ' месяцев</strong>. ';
    table += 'Ставка: <strong>' + base_rate + '% годовых</strong>.</h4>';
    if (payment_type <= 2) {
        table += '<p><a href="#" onclick="showTermCompare(); return false;"><span class="fa fa-pie-chart"></span> Сравнить платежи для разных сроков...</a></p>';
...
$(document).ready(function() {
    ...
    $('#calc_sum_range').on(rangeMethod1, calcSumRangeInput);
    ...
    $('.calc-shedule-show').click(function(event) {
        event.preventDefault();
        showShedule();
        trackGoal('SHEDULE');
    });
    ...
    $('.calc-preset').click(function() {
        ...
        $('#calc_sum_range').attr('min', min_sum);
...
function initCallbackButton() {
    $(document.body).append('<a href="#" id="callback__toggle"><div class="circlephone" style="transform-origin: center;"></div><div class="circle-fill" style="transform-origin: center;"></div><div class="img-circle" style="transform-origin: center;"><div class="img-circleblock" style="transform-origin: center;"></div></div></a>');
    $('#callback__toggle, .callback-btn2, .callback-link').click(function(event) {
        event.preventDefault();
        showCallbackForm();
    });
};
...
//         digits = digits.slice(0, 12); // Только для UA
//       }
...
//         }
//       } else {
//         formatted += digits; // Просто выводим как есть (другие страны)
//       }
...
//       // Фикс бэкспейса для корректного удаления
//       input.addEventListener("keydown", function (e) {
//         if (e.key === 'Backspace') {
//           const pos = input.selectionStart;
//           const val = input.value;
  
//           // Если удаляем спецсимвол (например, ")", "-", " ")
//           if (pos > 0 && /\D/.test(val[pos - 1])) {
//             e.preventDefault();
//             let newPos = pos - 1;
//             while (newPos > 0 && /\D/.test(val[newPos - 1])) {
//               newPos--;
//             }
  
//             const rawDigits = val.replace(/\D/g, '');
//             input.value = formatPhone(rawDigits.slice(0, rawDigits.length - 1));
...
//     document.querySelectorAll('.callback-btn2').forEach(function (btn) {
//       btn.addEventListener('click', function () {
//         setTimeout(function () {
//           const modalInputs = document.querySelectorAll('input[name="telephone"]');
//           modalInputs.forEach(applyPhoneMask);
//         }, 200);
//       });
//     });
//     document.body.addEventListener('submit', e => {
//         const form = e.target;
//         if (form.matches('form')) {  // например, нужная форма с классом callback-form
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    function applyPhoneMask(input) {
      ...
      // Показ маски при фокусе
      input.addEventListener("focus", function () {
        let digits = getRawDigits(input.value);
        if (digits.length === 0) {
          digits = '38';
        }
        input.value = formatPhone(digits);
        setTimeout(() => {
          ...
          input.setSelectionRange(firstUnderscore, firstUnderscore);
        }, 0);
      });
      ...
      input.addEventListener("paste", function (e) {
        e.preventDefault();
        const pasted = e.clipboardData.getData("text").replace(/\D/g, '');
        const digits = pasted.startsWith('38') ? pasted : '38' + pasted;
        input.value = formatPhone(digits);
    ...
    // Маска для всех инпутов
    const phoneInputs = document.querySelectorAll('input[name="telephone"]');
    phoneInputs.forEach(applyPhoneMask);
  
    // Если поле появляется позже (например, в модалке)
    document.querySelectorAll('.callback-btn2').forEach(function (btn) {
      btn.addEventListener('click', function () {
        setTimeout(function () {
          const modalInputs = document.querySelectorAll('input[name="telephone"]');
    ...
    // Приведение к числовому формату при submit
    document.body.addEventListener('submit', e => {
      const form = e.target;
      if (form.matches('form')) {
        const phoneInput = form.querySelector('input[name="telephone"]');
        if (phoneInput) {
          let digits = getRawDigits(phoneInput.value);
          if (digits.length === 10 && digits.startsWith('0')) {
            digits = '38' + digits;
          }
          if (!digits.startsWith('38')) {
